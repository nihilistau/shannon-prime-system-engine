/* test_qwen3_vulkan.c — M_QWEN3_VULKAN: the §8.3 Phase-2-VK E-tests on Qwen3-0.6B.
 *   E_VK_1  loader (qwen3_load).
 *   E_VK_2  forward distributional gate vs the stock-llama.cpp oracle (argmax +
 *           top-5 cross + mean KL) — same gate as E_CPU_2/E_CU_2/§8.6.1.
 *   E_VK_3  per-row Frobenius Q8 arena (SP_ARENA=q8), Vulkan vs CPU.
 *   E_VK_4  the tiled f32 GEMM compute shader is the Vulkan "vectorised" matmul
 *           path (exercised by all the above; true f32, no relaxed precision). */
...
static void M_QWEN3_VULKAN(void) {
    ...
    SP_CHECK(f != NULL, "open qwen3_ref.bin (E_VK_2 oracle)");
    ...
    SP_CHECK(m && m->cfg.arch == SP_ARCH_QWEN3, "qwen3 load (E_VK_1)");
    ...
    if (!cpu || !vk) { free(cpu); free(vk); free(toks); free(ref); qwen3_free(m); return; }

    int rc_cpu = qwen3_forward(m, toks, (int)nt, cpu);
    int rc_vk  = qwen3_forward_vulkan(m, toks, (int)nt, vk);
    SP_CHECK(rc_cpu == 0 && rc_vk == 0, "qwen3 forward cpu + vulkan");
    if (rc_vk) fprintf(stderr, "    sp_last_error: %s\n", sp_last_error());

    if (rc_cpu == 0 && rc_vk == 0) {
        double kl_sum = 0; long argmax_ok = 0, top5_ok = 0;
        for (uint32_t t = 0; t < nt; t++) {
            const float *gr = vk + (size_t)t * V, *rr = ref + (size_t)t * V;
            int gm = 0, rm = 0;
        ...
        fprintf(stderr, "    [E_VK_2 vulkan-vs-oracle] argmax=%ld/%u top5=%ld/%u KL mean=%.3e\n",
                argmax_ok, nt, top5_ok, nt, kl_sum / nt);
        SP_CHECK_EQ_I64(argmax_ok, nt, "E_VK_2: Vulkan argmax matches ggml oracle");
        SP_CHECK_EQ_I64(top5_ok, nt, "E_VK_2: top-1 each inside the other's top-5");
        SP_CHECK(kl_sum / nt < 1.0e-5, "E_VK_2: mean KL(oracle||vulkan) below 1e-5");

        compare("8.3 vulkan-vs-cpu f32", cpu, vk, nt, V, 1.0e-5);
    }
    qwen3_free(m);

    /* ── E_VK_3: per-row Frobenius Q8 arena, Vulkan vs CPU ── */
    ENV_SET("SP_ARENA", "q8");
    m = qwen3_load(SP_QWEN3_GGUF);
    SP_CHECK(m && m->arena && sp_arena_precision(m->arena) == 8, "qwen3 q8 arena load (E_VK_3)");
    if (m) {
        int r1 = qwen3_forward(m, toks, (int)nt, cpu);
        int r2 = qwen3_forward_vulkan(m, toks, (int)nt, vk);
        SP_CHECK(r1 == 0 && r2 == 0, "qwen3 q8 forward cpu + vulkan");
        if (r2) fprintf(stderr, "    sp_last_error: %s\n", sp_last_error());
        if (r1 == 0 && r2 == 0) compare("E_VK_3 q8 vulkan-vs-cpu", cpu, vk, nt, V, 1.0e-5);
        qwen3_free(m);
    }
    ENV_CLR("SP_ARENA");

    free(cpu); free(vk); free(toks); free(ref);
}

int main(void) { SP_RUN(M_QWEN3_VULKAN); return SP_DONE(); }
