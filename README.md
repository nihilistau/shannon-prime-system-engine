...
> `served 12B chat + W_c recall` **GREEN-LIVE** · `byte-exact forward` **gated-GREEN / default-off** · `NIGHTSHIFT offline curator` **gated-GREEN on synthetic (live PENDING)** · `native diffusion judge` **UNPROVEN / in-drawer** · `diffusion-judge perf` **~2x stacked, async byte-exact / default-off** · north-star `papers/DESIGN-COLA-DLM-MAPPING.md` (block-causal latent diffusion)

**License:** MIT (see `LICENSE`). **HEAD:** `2a1c830` (diffusion-judge perf: scratch-reuse default-on + async double-buffer byte-exact). Public site: [Position Is Arithmetic](https://nihilistau.github.io/Position_Is_Arithmetic/).
...
| **Whole-machine diffusion-judge throughput** -- MoE spillover-expert streaming: per-expert device scratch-pool reuse + pinned async double-buffer prefetch (overlap upload of expert N+1 with compute of expert N), ~2x stacked | **scratch-reuse PROVEN default-on; async byte-exact default-off (promotion HELD)** | `SP_DG_SCRATCHREUSE` (on) · `SP_DG_ASYNC` (off) · `SP_DG_MOECHK` (determinism oracle) | scratch-reuse ~1.46x reversed-A/B (`e31c70d`); async byte-exact, 6-item concurrency stress 1440/1440 MOECHK OFF==ON (`2a1c830`). **NOTE:** the earlier "judge-recall regression" framing is WITHDRAWN -- the killed native run's 8.3% is native-judge weakness (95.6% is the EXTERNAL oracle), NOT an OOB-fix regression; self-cond verified correctly wired. async is byte-exact + default-off || **Two-ring long-context memory (PPT-ARM)** — ±1 Rademacher recall router + byte-addressable KV offload to NVMe/Optane (CPU backend) | shipped + measured | `SP_RECALL_*` / `SP_RING2_*` | **910× resident-KV shrink @32k**, 7.57 µs/read off Optane, **8× sparsification @ +0.69% PPL**, bit-exact when off. **Honest negative kept on the board: the 32k NIAH finale MISSed** (raw router breaks the 64× selection-budget; Ring 3 is the designed fix) |
