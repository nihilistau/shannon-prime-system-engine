@echo off
REM Single-pass admission labeler: ablation gate (SP_B3_DISPOSER=2) with NO SP_B3_SECRET,
REM so each episode self-ablates using ITS OWN ep.secret sidecar. One daemon boot labels
REM the whole registry. %1 = registry jsonl. TAU left at -inf (telemetry only, no re-inject).
