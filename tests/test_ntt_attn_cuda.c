/* test_ntt_attn_cuda.c — E_CU_5: NTT-attention on CUDA.
 *
 * Part A (substitution lock, CPU): the CUDA NTT-attn kernel computes the score
 ...
 * which is why the GPU may skip the NTT (int64 holds the result; |ip| << 2^63).
 * The full CRT-NTT port is deferred (load-bearing only at Phase-6 cross-node
 * CRT sharding, where the polynomial product is split across coprime primes).
 *
 * Part B (gate, CUDA): qwen3_forward_cuda with SP_ENGINE_NTT_ATTN=1 vs the f32
...
/* deterministic xorshift for reproducible random int32 in [-range, range] */
...
static void E_CU_5(void) {
    /* ── Part A: int64 dot == sp_pr_inner (the substitution that lets the GPU
     * skip the CRT-NTT). ── */
    ...
    /* ── Part B: CUDA NTT-attn vs CUDA f32 forward, mean KL <= 1e-7, KL > 0. ── */
    SP_CHECK(sp_cuda_device_count() >= 1, "CUDA device visible");
    if (sp_cuda_device_count() < 1) return;
    ...
    int rb = qwen3_forward_cuda(m, toks, (int)nt, base);    /* f32 attention */
    ENV_SET("SP_ENGINE_NTT_ATTN", "1");
    int rn = qwen3_forward_cuda(m, toks, (int)nt, ntt);     /* NTT (exact-integer) attention */
    ENV_CLR("SP_ENGINE_NTT_ATTN");
    SP_CHECK(rb == 0 && rn == 0, "cuda forward f32 + ntt");
...
int main(void) { SP_RUN(E_CU_5); return SP_DONE(); }
