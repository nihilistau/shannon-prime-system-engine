...
- **Audio port (KAI-3, the bridge into the GNA "EAR" line):** **CLOSED GREEN** (engine `e35a227`). A *sequence* of N projected frames is injected via the new ABI `gemma4_kv_inject_seq` (G-KAIROS-3-NULL: 2/2 byte-identical to the inline EMB loop). The projector (`tools/audio_port/`) is a per-position MLP 640→V_sub with an on-manifold binder, trained with dense per-position CE; the run is **local / no-cloud** (the engine owns the gemma tokenizer via the new `SP_G4_TOK_DUMP`). Synthetic ladder at `noise_rel=0.1` → top1 1.000 / cos 0.9998; real-token V_sub=60 → top1 0.931 / cos 0.9937; the metal gate `SP_G4_KAI3` is **8/8 SEMANTIC pivots** (`KAI3_GATE_EXIT=0`). KAI-3 is the BRIDGE into the GNA audio "EAR" line — separate from but sharing the same `gemma4_kv_inject` residual-entry seam as the KAIROS latent-memory work.
...
| **Audio port / projector (KAI-3 → GNA "EAR")** | `tools/audio_port/` (per-position MLP 640→V_sub + on-manifold binder) + `tests/test_gemma4_cuda.c` (`SP_G4_INJ_SEQ`, `SP_G4_TOK_DUMP`, `SP_G4_KAI3`) | shipped + gated; KAI-3 **CLOSED GREEN** (8/8 semantic pivots, local/no-cloud). NEXT = GNA Stage 2: real GNA/CNN audio front-end replaces the synthetic anchor matrix |
...
| | GNA Stage 2 (task #154) — real audio front-end | NEXT | replace the synthetic anchor matrix A with the real GNA/CNN audio front-end (live audio → 40ms / 640-float / 16kHz frames; `audio_token_id=258881`). KAI-3 delivery + projection architecture LOCKED. (Near-term pivot; the project pivots BACK to XBAR / KAIROS latent memory afterward) |
