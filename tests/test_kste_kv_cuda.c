/* test_kste_kv_cuda.c — E_CU_6: KSTE KV-cache encoding on CUDA.
 *
 * qwen3_forward_cuda_ex encodes each post-norm/post-RoPE K head-vector to its
 * 64-byte KSTE signature via the host sp_kste_encode (the on-device kernel is
 * deferred). KSTE byte-identity ACROSS backends is structurally unachievable —
 * order-statistic encoding amplifies the same §8.6.1 cuBLAS-vs-scalar FP floor
 * that QK-norm amplifies for logits — so, exactly like E_CPU_2 became a
 * distributional gate, the cross-backend gate here is NOT byte-identity. Gates:
 *   Part A (encoder)  : sp_kste_encode is deterministic on fixed input — the
 *                       portable-across-backends property (same host fn). Hard.
 *   Part B (model K)  : CUDA signatures deterministic + wire-valid (the literal
 *                       E_CPU_6 gate). Hard.
 *   Proof of cause    : the Tier-0 ROOT order-statistic labels (6 int16) of the
 *                       CUDA vs CPU signatures barely move (max drift small) —
 *                       proving the divergence is FP-floor K boundary flips, not
 *                       a logic bug. Agreement rate reported (loose floor only). */
...
static void E_CU_6(void) {
    ...
    /* ── Part B: model-driven signatures, CPU vs CUDA ── */
    ...
    sp_kste_tree_t *kv_cu  = (sp_kste_tree_t *)calloc(ntrees, sizeof(sp_kste_tree_t));
    sp_kste_tree_t *kv_cu2 = (sp_kste_tree_t *)calloc(ntrees, sizeof(sp_kste_tree_t));
    float *logits = (float *)malloc((size_t)nt * V * sizeof(float));
    if (!kv_cpu || !kv_cu || !kv_cu2 || !logits) { SP_CHECK(0, "alloc"); goto cleanup; }

    {
        int rc_cpu = qwen3_forward_ex(m, toks, (int)nt, logits, kv_cpu);
        int rc_cu  = qwen3_forward_cuda_ex(m, toks, (int)nt, logits, kv_cu);
        int rc_cu2 = qwen3_forward_cuda_ex(m, toks, (int)nt, logits, kv_cu2);
        SP_CHECK(rc_cpu == 0 && rc_cu == 0 && rc_cu2 == 0, "cpu + cuda(x2) forward_ex");
        if (rc_cu) fprintf(stderr, "    sp_last_error: %s\n", sp_last_error());
        if (rc_cpu || rc_cu || rc_cu2) goto cleanup;

        /* deterministic + wire-valid (the literal E_CPU_6 gate) */
        int det = memcmp(kv_cu, kv_cu2, ntrees * sizeof(sp_kste_tree_t)) == 0;
        SP_CHECK(det, "E_CU_6: CUDA signatures deterministic across runs");
        long nonzero = 0, bad_ver = 0;
        for (size_t i = 0; i < ntrees; i++) {
            if (kv_cu[i].bytes[SP_KSTE_OFF_VERSION] != (uint8_t)SP_KSTE_LAYOUT_VERSION) bad_ver++;
            for (int b = 0; b < 64; b++) if (kv_cu[i].bytes[b]) { nonzero++; break; }
        }
        SP_CHECK_EQ_I64(bad_ver, 0, "E_CU_6: every signature carries the frozen layout version");
        SP_CHECK(nonzero > 0, "E_CU_6: signatures non-trivial (encode ran)");

        /* cause proof: agreement rate + Tier-0 root-label drift (6 int16) */
        long same = 0; int max_label_drift = 0; double sum_label_drift = 0; long ncmp = 0;
        for (size_t i = 0; i < ntrees; i++) {
            if (memcmp(&kv_cpu[i], &kv_cu[i], sizeof(sp_kste_tree_t)) == 0) same++;
            const uint8_t *pc = kv_cpu[i].bytes + SP_KSTE_OFF_ROOT;
            const uint8_t *pg = kv_cu[i].bytes  + SP_KSTE_OFF_ROOT;
            for (int c = 0; c < 6; c++) {
        ...
        /* loose floor: < 50% with a deterministic CUDA + identical encoder would
         * mean the K path is wrong (not just FP-floor boundary flips). */
        SP_CHECK(agree > 0.50, "E_CU_6: cross-backend agreement > 50% (rest = K-quantization boundary flips)");
        /* the cause proof: order-statistic labels barely move => FP-floor K, not a
         * logic bug. A real K discrepancy would move the int16 labels by >> 1 LSB. */
        SP_CHECK(max_label_drift <= 4, "E_CU_6: Tier-0 order-statistic labels within 4 LSB of CPU (FP-floor cause)");
    }

cleanup:
    free(kv_cpu); free(kv_cu); free(kv_cu2); free(logits);
    free(toks); qwen3_free(m);
}

int main(void) { SP_RUN(E_CU_6); return SP_DONE(); }
