@echo off
REM G-QKEYS-MARGIN — V3 gate with systemecho delivery (the proven prior-override framing).
REM Arg1: "head" = SP_SPECTEST + SP_SPECTEST_HEAD on; anything else = nohead baseline.
REM Arg2 (optional): SP_RECALL_L5_MARGIN value (default 0.0 = margin-NULL off).
REM Registry frozen (no B4 growth). Delivery = systemecho (overrides prior knowledge).
setlocal
set "ENGINE=%~dp0"
...
set "SP_QKEY_MINT=1"
set "SP_RECALL_L5_MARGIN=%~2"
if "%~2"=="" set "SP_RECALL_L5_MARGIN=0.0"
set "CUBLAS_WORKSPACE_CONFIG=:16:8"
if "%~1"=="head" (
    set "SP_SPECTEST=1"
    set "SP_SPECTEST_HEAD=%ENGINE%_faithful_corpus\f3\spectest_head_f1.bin"
)
set "SP_DAEMON_LOG=%ENGINE%_v3_serve_se.log"
echo [V3-gate-SE] systemecho delivery, mode=%~1 margin=%SP_RECALL_L5_MARGIN%
