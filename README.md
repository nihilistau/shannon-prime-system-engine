...
**The current edge (2026-06-20): the served Gemma-4-12B runs the full
discrete stack with an AUTONOMOUS LIBRARIAN on top.** `run_console_recall.bat`
→ http://127.0.0.1:3000/ chats the real 12B — **coherent + byte-exact +
O(1)-context** (the SWA ring + single inject seam — the CHAT-FULLSTACK stack) —
and arms a **learned `W_c` recall head** that does autonomous, instance-level
episodic memory: every turn is scored against the registry, an (E+1)-way argmax
over `[episodes, NULL]` either **replays the winning episode** (bounded mass
@`SP_REPLAY_MTARGET=42`) or **rejects to a clean prompt**. Live gate
**G-CHAT-B3-WC-DEPLOY** (engine `edc8079`): a matched query recalls
`ep_n_div_000`, a foreign query ("capital of France?") rejects to NULL →
clean "Paris." Default-off (`SP_B3_WC` unset) = byte-identical null floor.
See [§2 status](#2-current-status--honest-table), [§3.6 quickstart](#36-served-chat--the-autonomous-librarian), [§9.2 env](#92-runtime-knob-env-vars-read-by-math-core--backends-per-forward).
...
- **Autonomous librarian (the served chat — B3-WC, CLOSED LIVE on the 12B):** the full stack is served end-to-end. **CHAT-FULLSTACK** (`run_console_recall.bat` / `run_console.bat` → http://127.0.0.1:3000/) chats the real Gemma-4-12B **coherent + byte-exact + O(1)-context** on the single latent inject seam (`gemma4_kv_inject_seq`; text-via-seam == prefill bit-identical). On top, a learned **`W_c` recall head** (`tools/sp_daemon/src/recall.rs` `WcHead`/`load_wc`/`wc_score`; HD=512→r=32, relevance = **logsumexp-over-positions then mean-over-heads**) does autonomous instance-level recall: `routes.rs` `SP_B3_WC` scores every registry episode, takes the **(E+1)-way argmax over `[episodes, NULL=s0]`**, and either replays the winner @`SP_REPLAY_MTARGET=42` (bounded mass) or rejects to a clean prompt. Gate **G-CHAT-B3-WC-DIV2**: 360/361 recall + 50/50 foreign-reject, int16==f32, s0=+0.102; **G-CHAT-B3-WC-DEPLOY** LIVE (engine `edc8079`). Foundation: the teacher-forced ablation knockout (`SP_B3_SECRET`, novel −33.56 vs parametric −0.15, TAU=−8.0) is both the admission oracle and the head's labeler; curator `tools/xbar_lsh/mint_corpus*.py`; corpus diversity took instance recall **34%→100%** (`f62e6ef`). Default-off (`SP_B3_WC` unset) = byte-identical null floor.
...
|-------|-----------|:------:|------------------|
| **Autonomous librarian (2026-06-20)** | B3-WC learned recall head LIVE on the served 12B (`SP_B3_WC`) | CLOSED LIVE | **G-CHAT-B3-WC-DEPLOY** (engine `edc8079`): `recall.rs` `WcHead`/`wc_score` (HD=512→r=32, **logsumexp-over-positions then mean-over-heads**) + `routes.rs` `SP_B3_WC` branch — score every registry episode, **(E+1)-way argmax over `[episodes, NULL=s0]`**, replay winner @`SP_REPLAY_MTARGET=42` (bounded mass) or reject to a clean prompt. Live on the 90-needle div registry: matched → RECALL `ep_n_div_000` (9.858), foreign → NULL → clean "Paris." Deploy blob `_b3_wc/wc_deploy.bin` (WCB1) via `tools/xbar_lsh/export_wc_deploy.py`. Default-off (env unset) = byte-identical null floor |
| | B3-WC head — train + reject gate (`G-CHAT-B3-WC-DIV2`) | CLOSED GREEN | **360/361 recall + 50/50 foreign-reject, int16==f32 lossless, s0=+0.102** (engine `87044d8`). The win is a **learned head on a DIVERSE corpus**, not a hand-designed signal — corpus diversity took instance recall **34%→100%** (`f62e6ef`); every number-theoretic / hand-designed relevance lever is a measured negative (boundary thesis) |
| | B3-WC foundation — teacher-forced ablation knockout (`SP_B3_SECRET`) | CLOSED GREEN | the admission **oracle + the head's labeler**: `cudaMemset`-ablate (`gemma4_kv_ablate_rows`) the secret's source KV rows, teacher-force the known secret, ΣΔLL with/without → **novel −33.56 vs parametric −0.15**, 3-archetype matrix **TAU=−8.0** (engine `15738c1`/`b6470cc`). Curator `tools/xbar_lsh/mint_corpus*.py` + single-pass ep.secret admission |
| | CHAT-FULLSTACK — coherent + byte-exact + O(1) served 12B chat | CLOSED GREEN | `run_console.bat` / `run_console_recall.bat` → http://127.0.0.1:3000/ chats the real 12B on the **single latent inject seam** (`gemma4_kv_inject_seq`; text-via-seam == prefill bit-identical), coherent + reproducible + byte-exact + O(1)-context (SWA ring). Stages S1/A2/B1/B5/B2-ring all GREEN. Lattice `CONTRACT-CHAT-FULLSTACK` |
...
**2026-06-20 — the AUTONOMOUS LIBRARIAN is LIVE on the served 12B chat.** A learned **`W_c` head** (`tools/sp_daemon/src/recall.rs` `WcHead`/`load_wc`/`wc_score`; relevance = **logsumexp-over-positions then mean-over-heads** through an HD=512→r=32 projection) does autonomous, instance-level episodic recall on `/v1/chat`: `routes.rs` `SP_B3_WC` branch scores every registry episode, takes the **(E+1)-way argmax over `[episodes, NULL=s0]`**, and replays the winner @`SP_REPLAY_MTARGET=42` (bounded injection mass) or rejects to a clean prompt; default-off (env unset) = byte-identical null floor. **Gates:** **G-CHAT-B3-WC-DIV2** (`87044d8`) = **360/361 recall + 50/50 foreign-reject, int16==f32 lossless, s0=+0.102**; **G-CHAT-B3-WC-DEPLOY** (`edc8079`, LIVE on the metal) = matched → RECALL `ep_n_div_000` (9.858), foreign → NULL → clean "Paris." The relevance foundation is the **teacher-forced ablation knockout** (`SP_B3_SECRET`: `cudaMemset`-ablate the secret's source KV rows via `gemma4_kv_ablate_rows`, teacher-force the known secret, ΣΔLL with/without → novel **−33.56** vs parametric **−0.15**, 3-archetype matrix TAU=−8.0, `15738c1`/`b6470cc`) which is both the admission oracle and the head's labeler; the curator is `tools/xbar_lsh/mint_corpus*.py` + the single-pass ep.secret admission. Deploy blob `_b3_wc/wc_deploy.bin` (WCB1) via `tools/xbar_lsh/export_wc_deploy.py`; launch `run_console_recall.bat` (recall armed, registry `_needle_corpus_div`, M=42); the console **Send** button toggles to an **■ stop** interrupt (POST `/v1/abort/:id`, session `cancel_flag`). **Boundary thesis extends:** autonomous recall is won by a LEARNED head on a DIVERSE corpus, not hand-designed signals (every one a measured negative; corpus diversity was the binding constraint — instance recall **34%→100%**, `f62e6ef`). Commits `edc8079` (deploy) / `87044d8` (head) / `f62e6ef` (diversity) / `fc25b07` (console stop button) / `89832dd` (CLAUDE). Detail: lattice `CONTRACT-CHAT-FULLSTACK` + `SESSION-HANDOFF.md §0d`.
...
### 3.6 Served chat + the autonomous librarian (`run_console_recall.bat`)

The CHAT-FULLSTACK served chat — the real Gemma-4-12B, coherent + byte-exact +
O(1)-context — plus the **B3-WC autonomous librarian** is a single launcher:

```cmd
:: coherent byte-exact chat, served at http://127.0.0.1:3000/
run_console.bat

:: same chat + the LEARNED-HEAD autonomous recall ARMED (registry _needle_corpus_div, M=42)
run_console_recall.bat
```

Open the browser at **http://127.0.0.1:3000/** once the daemon logs `listening`.
`run_console_recall.bat` is `run_console.bat` plus exactly three env vars — the
B3-WC recall delta (see [§9.2](#92-runtime-knob-env-vars-read-by-math-core--backends-per-forward)):
...
Watch the daemon console for `B3-WC ... RECALL '<ep>'` (matched) or
`NULL wins -> REJECT` (clean prompt). On the served console the **Send** button
toggles to an **■ stop** interrupt while generating — it fires a client-side
abort and `POST /v1/abort/:id` (backed by the session `cancel_flag`); `Esc` /
`Ctrl+C` also interrupt. Both launchers are static-asset / env-only — **no
rebuild**, and with `SP_B3_WC` unset the recall path is a byte-identical null
floor.
...
│                                                                           │
│  recall.rs       ── B3-WC autonomous librarian: WcHead / wc_score        │
│     query-Q ▶ wc_score(ep) ∀ep ▶ (E+1) argmax[episodes, NULL=s0]      │
│               ├─ episode wins ▶ kv::replay @ SP_REPLAY_MTARGET=42       │
│               └─ NULL wins    ▶ clean prompt (reject)                   │
...
| `SP_RECALL_REGISTRY` | path | Episode registry JSONL loaded at startup (one row per episode: name / dir / npos / gk). Required for any autonomous recall; unset = no registry, recall is a no-op |
| `SP_B3_WC` | path | (B3-WC autonomous librarian) the learned `W_c` deploy blob (`wc_deploy.bin`, WCB1). When set, `/v1/chat` scores each registry episode by logsumexp-mean relevance and does the (E+1) argmax over `[episodes, NULL=s0]` — episode wins → replay, NULL wins → clean prompt. **Unset = byte-identical null floor.** Built by `tools/xbar_lsh/export_wc_deploy.py` |
| `SP_REPLAY_MTARGET` | int | Bounded injection-mass budget (M_target) for a recalled episode's replay — clamps the recalled memory's attention mass so a wrong fire stumbles rather than hijacks. The deployed operating point is `42` |
