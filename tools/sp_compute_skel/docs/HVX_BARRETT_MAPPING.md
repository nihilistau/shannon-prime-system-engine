...
|---|---|---|---|---|---|
| 1 | u30 × u30 → u60 widening, low 32 → Vdd.v[0] | `Q6_W_vmpye_VwVuh(va, vb)` | `Vdd32 = vmpye(Vu32.w, Vv32.uh)` | hvx_hexagon_protos.h:~2148 | TBD Stage 4 |
| 2 | (cont.) widening high 32 → Vdd.v[1], accumulating | `Q6_W_vmpyoacc_WVwVh(pair, va, vb)` | `Vxx32 += vmpyo(Vu32.w, Vv32.h)` | hvx_hexagon_protos.h:2381 | TBD Stage 4 |
| 3 | Extract low 32-bit word per lane from pair | `Q6_V_lo_W(pair)` | (compiler synth — no opcode) | hvx_hexagon_protos.h:41 | TBD Stage 4 |
| 4 | Extract high 32-bit word per lane from pair | `Q6_V_hi_W(pair)` | (compiler synth — no opcode) | hvx_hexagon_protos.h:32 | TBD Stage 4 |
| 5 | Logical shift right u32 by 29 (for x>>29 lo half) | `Q6_Vuw_vlsr_VuwR(x_lo, 29)` | `Vd32.uw = vlsr(Vu32.uw, Rt32)` | hvx_hexagon_protos.h:1751 | TBD Stage 4 |
| 6 | Arithmetic shift left u32 by 3 (for x_hi<<3 in >>29 assembly) | `Q6_Vw_vasl_VwR(x_hi, 3)` | `Vd32.w = vasl(Vu32.w, Rt32)` | hvx_hexagon_protos.h:725 | TBD Stage 4 |
| 7 | Bitwise OR (combine shifted hi + shifted lo) | `Q6_V_vor_VV(sh_lo_shifted, hi_shifted)` | `Vd32 = vor(Vu32, Vv32)` | hvx_hexagon_protos.h:2579 | TBD Stage 4 |
| 8 | u31 × u31 → u62 widening (sh × mu), low 32 | `Q6_W_vmpye_VwVuh(sh, vmu)` | `Vdd32 = vmpye(Vu32.w, Vv32.uh)` | (same as #1) | TBD Stage 4 |
| 9 | (cont.) widening high 32, accumulating | `Q6_W_vmpyoacc_WVwVh(pair, sh, vmu)` | `Vxx32 += vmpyo(Vu32.w, Vv32.h)` | (same as #2) | TBD Stage 4 |
| 10 | Logical shift right by 31 (qhat = q_pair >> 31, lo) | `Q6_Vuw_vlsr_VuwR(q_lo, 31)` | `Vd32.uw = vlsr(Vu32.uw, Rt32)` | (same as #5) | TBD Stage 4 |
| 11 | Shift left by 1 (q_hi << 1, for >>31 assembly) | `Q6_Vw_vasl_VwR(q_hi, 1)` | `Vd32.w = vasl(Vu32.w, Rt32)` | (same as #6) | TBD Stage 4 |
| 12 | qhat × q, only low 32 bits (Vdd.v[0]) | `Q6_W_vmpye_VwVuh(qhat, vq)` | `Vdd32 = vmpye(Vu32.w, Vv32.uh)` | (same as #1) | TBD Stage 4 |
| 13 | (cont.) qhat × q accumulator | `Q6_W_vmpyoacc_WVwVh(pair, qhat, vq)` | `Vxx32 += vmpyo(Vu32.w, Vv32.h)` | (same as #2) | TBD Stage 4 |
| 14 | r0 = x_lo - qq_lo (modular sub) | `Q6_Vw_vsub_VwVw(x_lo, qq_lo)` | `Vd32.w = vsub(Vu32.w, Vv32.w)` | hvx_hexagon_protos.h:3326 | TBD Stage 4 |
| 15 | Unsigned-gt compare (r0 > q-1) for first Barrett correction | `Q6_Q_vcmp_gt_VuwVuw(r0, vq_minus_1)` | `Qd4 = vcmp.gt(Vu32.uw, Vv32.uw)` | hvx_hexagon_protos.h:1625 | TBD Stage 4 |
| 16 | r1 = (mask ? r0 - q : r0) | `Q6_V_vmux_QVV(gt0, vsub(r0, vq), r0)` | `Vd32 = vmux(Qt4, Vu32, Vv32)` | hvx_hexagon_protos.h:2507 | TBD Stage 4 |
| 17 | (second Barrett correction — same intrinsics #15-16 applied to r1) | (same) | (same) | (same) | TBD Stage 4 |
| 18 | Splat u32 constants (q, q-1, mu) — load-time, hoisted | `Q6_V_vsplat_R(Rt)` | `Vd32 = vsplat(Rt32)` | hvx_hexagon_protos.h:68 | TBD Stage 4 |
